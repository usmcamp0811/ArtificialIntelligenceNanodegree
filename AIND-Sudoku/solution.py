...
def display(values):
    ...
    """
    width = 1+max(len(values[d]) for d in boxes)
    line = "+".join(['-'*(width*3)]*3)
    for r in rows:
...
def eliminate(values):
    completed_cells = [cell for cell in values.keys() if len(values[cell]) == 1] # get a list of cells that are done
